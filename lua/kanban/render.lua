...
local function create_window(window)
  local width = math.floor(vim.o.columns * window.width)
  local height = math.floor(vim.o.lines * window.height)
...
local function calculate_column_width(win_width, num_columns)
  ...
  local gaps = num_columns - 1
  local available = win_width - gaps
...
local function render_row(board, row_index, col_width, separator)
  local parts = {}
  local highlights = {}
  local current_pos = 0
  ...
  for col_idx, column in ipairs(board.columns) do
    ...
    local is_header_row = row_index == 1
...
function M.render(board)
  ...
  -- NOTE: init.lua already validates the board before calling render
  ...
  local header_row = 1
  local total_rows = header_row + max_tasks

  for i = 1, total_rows do
    local line, hl = render_row(i, col_width, separator)
  ...
  local available_height = win_height - (title_height + footer_height)
  ...
  for i, hls in pairs(highlights) do
...
function M.open(cfg, board)
  setup_highlights(cfg.highlights)
  M.buf, M.win = create_window(cfg.window)
  M.render(board)
end
